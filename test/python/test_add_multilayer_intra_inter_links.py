...
def test_add_multilayer_intra_links_accepts_non_contiguous_numpy_array(make_infomap):
    np = pytest.importorskip("numpy")
    links = np.array(
        [
            [1, 1, 2, 1.0, 99.0],
...
def test_add_multilayer_inter_links_accepts_non_contiguous_numpy_array(make_infomap):
    np = pytest.importorskip("numpy")
    links = np.array(
        [
            [1, 1, 2, 0.5, 99.0],
