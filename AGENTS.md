...
- `test/fixtures/expected/golden-codelengths.json` is generated by the CLI (the
  reference implementation) and asserted three-way by the CLI (`golden_codelengths`
  ctest gate), Python (`test_golden_codelengths.py`), and R
  (`test-golden-codelengths.R`). After an intentional algorithm/codelength change,
  regenerate with `make build-golden-codelengths` (the diff records the change)
  and confirm with `make test-golden-codelengths-freshness`
