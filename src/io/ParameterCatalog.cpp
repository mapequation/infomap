...
const std::vector<ParameterSpec>& parameterCatalog()
{
  static const std::vector<ParameterSpec> parameters = {
    ...
    param()
        .longName("intra-regularization-strength")
        .description("Scale the intra-layer part of the multilayer prior used by --regularized, relative to --regularization-strength. Set to 0 to regularize only between layers.")
        .argument(ArgType::number)
        .group("Algorithm")
        .advanced()
        .defaultValue("1")
        .min("0")
        .configTarget(&Config::intraRegularizationStrength),
    param()
        .longName("inter-regularization-strength")
        .description("Scale the inter-layer part of the multilayer prior used by --regularized, relative to --regularization-strength. Set to 0 to leave the layers uncoupled.")
        .argument(ArgType::number)
        .group("Algorithm")
        .advanced()
        .defaultValue("1")
        .min("0")
        .configTarget(&Config::interRegularizationStrength),
    param()
        .longName("multilayer-skip-absent-nodes")
        .description("Treat a physical node's absence from a layer as real rather than unobserved, so the prior network of --regularized spans only the nodes each layer holds.")
        .group("Algorithm")
        .advanced()
        .configTarget(&Config::multilayerSkipAbsentNodes),
    param()
