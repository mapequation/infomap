...
const std::vector<ParameterSpec>& parameterCatalog()
{
  static const std::vector<ParameterSpec> parameters = {
    ...
    param()
        ...
        .description("Use batched parallel node moves for coarse optimization; usually beneficial only with a relaxed core-loop-codelength-threshold and low tune-iteration-limit, and may produce a different partition than serial optimization.")
        .group("Accuracy")
        .advanced()
