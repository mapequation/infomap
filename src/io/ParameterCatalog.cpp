...
const std::vector<ParameterSpec>& parameterCatalog()
{
  static const std::vector<ParameterSpec> parameters = {
    ...
    param()
        .longName("multilayer-self-inter-links")
        .description("For inter/intra format, restrict inter-layer links within same physical node but adjust flow to approximate physical steps")
        .group("Input")
        .advanced()
        .configTarget(&Config::multilayerSelfInterLinks),
    param()
    ...
    param()
        .longName("hard-partition")
        .description("Do not split initial partition.")
        .group("Input")
        .advanced()
        .configTarget(&Config::clusterDataIsHard),
    param()
        .longName("out-name")
    ...
    param()
        .longName("random-node-check-rate")
        .description("Check a selected proportion of nodes for moves if recorded teleportation")
        .argument(ArgType::probability)
        .group("Algorithm")
        .advanced()
        .defaultValue("0")
        .range("0", "1")
        .bindingDefaults("None", "NULL")
        .configTarget(&Config::randomNodeCheckRate),
    param()
    ...
    param()
        .longName("multilayer-test")
        .description("Testing different multilayer implementations.")
        .argument(ArgType::integer)
        .group("Algorithm")
        .advanced()
        .defaultValue("0")
        .bindingDefaults("None", "NULL")
        .configTarget(&Config::multilayerTest),
    param()
        .longName("multilayer-aggregation")
        .description("Experimental: Use aggregated multilayer network.")
        .group("Algorithm")
        .advanced()
        .configTarget(&Config::multilayerAggregation),
    param()
        .shortName('s')
        .longName("seed")
