...
bool Infomap::haveMemory() const {
	return this->isMemoryNetwork();
}

InfomapBase* Infomap::getNewInfomapInstance() const {
	return new Infomap();
}

InfomapBase* Infomap::getNewInfomapInstanceWithoutMemory() const {
	return new Infomap(true);
}

double Infomap::getCodelength() const
{
	return m_objective->codelength;
}

double Infomap::getIndexCodelength() const
{
	return m_objective->indexCodelength;
}

double Infomap::getModuleCodelength() const
{
	return m_objective->moduleCodelength;
}

unsigned int Infomap::numActiveModules() const
{
...
void Infomap::initNetwork()
{
	Log(4) << "Infomap::initNetwork()...\n";
...
void Infomap::initSuperNetwork()
{
	Log(4) << "Infomap::initSuperNetwork()...\n";
	m_objective->initSuperNetwork(root());
}

double Infomap::calcCodelength(const InfoNode& parent) const
{
...
void Infomap::initPartition()
{
	auto& network = activeNetwork();
	Log(4) << "Infomap::initPartition() with " << network.size() << " nodes..." << std::endl;
...
void Infomap::moveActiveNodesToPredifinedModules(std::vector<unsigned int>& modules)
{
	auto& network = activeNetwork();
	unsigned int numNodes = network.size();
...
unsigned int Infomap::optimizeActiveNetwork()
{
...
// unsigned int Infomap::tryMoveEachNodeIntoBestModuleLocal()
// {
...
unsigned int Infomap::tryMoveEachNodeIntoBestModule()
{
	// m_rand.seed(123);
	// Get random enumeration of nodes
	std::vector<unsigned int> nodeEnumeration(activeNetwork().size());
 ...
 */
unsigned int Infomap::tryMoveEachNodeIntoBestModuleInParallel()
{
	// Get random enumeration of nodes
	std::vector<unsigned int> nodeEnumeration(activeNetwork().size());
	m_rand.getRandomizedIndexVector(nodeEnumeration);
...
void Infomap::consolidateModules(bool replaceExistingModules)
{
...
bool Infomap::restoreConsolidatedOptimizationPointIfNoImprovement(bool forceRestore)
{
...
std::ostream& Infomap::toString(std::ostream& out) const
{
