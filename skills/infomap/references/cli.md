...
Do not treat this skill as the CLI manual. Before you give runnable commands, inspect the installed binary:
...
If `infomap` is not found, try `Infomap`. Installed Python packages often provide `infomap`. Native builds can expose `Infomap`.
...
When the binary name is uncertain, write examples with `INFOMAP_BIN` and verify current option names from help before you run them.
...
- For validation, use tiny inputs or one trial first. Ask before you launch large files, many trials, multiple seeds, parameter sweeps, notebooks, or runs likely to take more than a few minutes.
...
- For link lists, state/multilayer input, metadata, bipartite data, and output formats, verify the installed help or published guide before you make format-specific claims.
...
- Choose native output formats only after you check current help, because option names and writer behavior are version-sensitive.
- When a user cites an old command from a discussion or paper, do not copy it verbatim. Translate it through current `--help`, `-hh`, and `--print-json-parameters`.
