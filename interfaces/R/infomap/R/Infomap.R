...
#' R6 class generator for Infomap (advanced use)
#'
#' Most users should call [Infomap()] instead. This generator is exposed
#' for subclassing and `$set()`-based extension.
#'
#' @keywords internal
InfomapClass <- R6::R6Class(
  ...
  public = list(
    ...
    #' @param names A list of `c(node_id, name)` vectors, or a named
    #'   list/vector mapping `node_id` to `name`.
    set_names = function(names) {
      if (is.null(base::names(names))) {
        for (entry in names) self$set_name(entry[[1L]], entry[[2L]])
      } else {
        for (id_str in base::names(names)) {
          self$set_name(as.integer(id_str), names[[id_str]])
        }
      }
      invisible(self)
    },
    ...
    #'   (weight optional), or a 2- or 3-column matrix / data.frame.
    add_links = function(links) {
      if (is.matrix(links) || is.data.frame(links)) {
        for (i in seq_len(nrow(links))) {
          row <- as.numeric(unlist(links[i, , drop = FALSE]))
          do.call(self$add_link, as.list(row))
        }
      } else {
        for (entry in links) {
