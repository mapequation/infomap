...
def _version_lt(a, b):
    """Return True if version string a < b, using simple numeric tuple comparison."""
    try:
        at = tuple(int(x) for x in str(a).split(".") if x.isdigit())
        bt = tuple(int(x) for x in str(b).split(".") if x.isdigit())
        return at < bt
    except Exception:
        return False
...
if macos_target is not None and "MACOSX_DEPLOYMENT_TARGET" not in os.environ:
    if _version_lt(macos_target, "10.9"):
