...
### F55 — Planted vs search across the whole overlapping family, at both trigram densities (2026-09-09)

Prompted by two holes in the benchmark set: om3 and om7 were missing, and every om existed at one
trigram density only (om2 at E50000, the rest at E100000) although the regularized failures had not
repeated across densities — om2 `--regularized` failed at E50000, om4 / om5 at E100000 (F47). A third
question came with it: the snapshot's planted rows were labelled `--no-infomap -c`, which reads as an
undirected evaluation. They were not — every planted run in `dissolve-ab-results.tsv` carries `-2d`;
only the label dropped it. The label now says `-2d --no-infomap -c`.

**Generator and provenance.** The five existing files are Andrea Lancichinetti's trigram sampler
(`build_syn_network` in the higher-order-regularization paper project, "Andrea's code"; the file stem is
its `get_suffix`). It is not an LFR run and not the `networks` package, whose `generate.lfr(..., on, om)`
makes first-order overlapping graphs only. The original is unseeded (Python `random`, no seed), so
"sample1" of those five cannot be regenerated byte for byte and nothing in this log records who ran it.
The nine new files — om3 / om7 at E100000, om2 at E100000, om3–om8 at E50000 — come from a line-by-line
port with `random.seed(sample)` added, `columnar_wip/make-om-network.py`; a throwaway om2 E50000 sample
from the port has the same shape as the original (8 × 64 cover, 28 008 states against 28 203).

**Measurement.** All 14 networks on one binary (tip `d9210b51`, md5 `b0b878cca7afa6976779d02cb3851fb1`,
`MODE=release OPENMP=0`), `-C --seed 123`: planted (`-2d --no-infomap -c`), one-level (printed in every
summary), free `-2d -N10`, free `-d -N10`, plain and `--regularized` — 84 runs, single pass, engine
`timing.total_s`. Full tables in `benchmark-networks.md`; rows in `columnar-search-runs.tsv`, batch
`om-family-summary`. Times are not a comparison instrument here (one run each on a loaded machine,
load 7–9): E100000 searches 5.1–12.2 s, E50000 searches 3.7–9.4 s, planted evaluations 0.08–0.31 s.

What the two densities show:

- **E100000 is the recovery regime.** Planted is 12.7–14.8% below one-level on the plain objective, and
  2.5–12.5% below it regularized for om ≤ 5. The free search lands 0.13–1.34% (plain) and 0.21–0.37%
  (regularized, om2–om4) below planted, with 60–921 top modules against 8–32 planted — the memory
  objective's optimum is finer than the planted cover on every row. om2 E100000 `--regularized` is the
  first row where the regularized search reaches the planted *scale*: 9 top modules against 8 planted,
  6.950 against 6.965 bits.
- **E50000 for om ≥ 3 is a different regime.** 1.40 → 0.97 links per state. The plain optimum is a
  forest of thousands of modules (om8: `-2d` 3.859 with 7 985 modules, `-d` 2.883 with 4 367 at 5
  levels, against planted 7.034 and one-level 7.991), so planted is no reference; under `--regularized`
  planted is worse than one-level on all six (8.14 → 9.61 against 7.97 → 7.99) while the search sits
  0.06–0.5% below one-level with 8–49 modules. These rows guard the fragmentation path of the search,
  not recovery of the cover.
- **One planted-vs-search gap remains: om5 E100000 `--regularized`, +2.26% bits** (7.9675 against
  planted 7.7918; `-d` 7.9650, +2.22%), unchanged since F47 and tracked as #1042. om2 and om4
  `--regularized`, failures in F47, are now 0.46% and 0.36% below planted; om3 and om7 are below planted
  at both densities on both objectives.
- **One hierarchical-vs-two-level gap on this binary: om8 E100000 plain `-d` 6.959 against `-2d`
  6.887 (+1.05% bits, 203 modules at 4 levels against 921 at 2).** #1041 recorded `-d` 4–14% worse
  than `-2d` on all five networks; on this binary every other row has `-d` at or below `-2d`, and at
  E50000 the hierarchy is where the gain is (om8: −25% bits from `-2d` to `-d`). #1041's seeded
  6.718 on om8 still stands, so the row's true gap is 3.6%, not 1%.
- om7 E100000 confirms F47's om6 / om8 observation as a trend: planted `--regularized` is worse than
  one-level from om6 up (8.03, 8.13, 8.29 against 7.99).
